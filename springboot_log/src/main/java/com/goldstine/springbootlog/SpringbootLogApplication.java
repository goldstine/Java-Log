...
public class SpringbootLogApplication {
	 ...
	 *
	 *相关的配置文件基本都是关于  ： 输出日志的格式配置 ;输出日志的位置 【控制台还是日志文件或者是数据库】  ；对应的root Logger   可以自己进行对应的包的Logger日志记录器对象的配置
	 * 相关的细粒度配置：将日志文件进行拆分规则，【按照日志文件的大小；按照日志文件的时间进行拆分】 通过日志级别进行过滤配置   通过异步日志文件的方式加快性能   通过自定义日志Logger对象进行日志输出
	 *
